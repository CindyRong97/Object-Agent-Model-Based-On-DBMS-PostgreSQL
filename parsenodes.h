...
{
  NodeTag type;
  char    *classname;   //创建的类的名字，此例中为student
  bool    isdeputy;     //是否是创建的代理类，此例为false
  List    *attrs;     //命令中声明的实属性列表，此例中为name和major_id
  //以下域只对定义代理类有效
  Node    *selectstmt; //定义代理类的
  Node    *createstmt; //创建的模式节点，由转换器生成
  char    *deputy_desc;//代理规则的描述
} CreateClassStmt;
